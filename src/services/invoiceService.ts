...
export async function autoGenerateInvoice(order_id: string, supplier_id: string, buyer_id: string) {
  ...
  return supabase.from('invoice').insert({
    order_id: order_id,
...
export async function getInvoicesForUser(userId: string) {
  ...
  return supabase.from('invoice').select('*').or(`supplier_id.eq.${userId},buyer_id.eq.${userId}`);
}

export async function getInvoiceById(invoiceId: string) {
  return supabase.from('invoice').select('*').eq('id', invoiceId).single();
}
